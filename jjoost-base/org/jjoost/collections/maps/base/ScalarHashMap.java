...
public class ScalarHashMap<K, V, N extends Entry<K, V>> extends AbstractHashMap<K, V, N> implements ScalarMap<K, V> {
	...
	public V put(K key, V val) {
		return table.put(key, nodeFactory.node(hash(key), key, val), keyEq, valProj()) ;
	}

	@Override
	public V putIfAbsent(K key, V val) {
		return table.putIfAbsent(key, nodeFactory.node(hash(key), key, val), keyEq, valProj()) ;
	}

	@Override
	public V ensureAndGet(K key, final Factory<? extends V> putIfNotPresent) {
		return table.ensureAndGet(hash(key), key, keyEq, new HashNodeFactory<K, N>() {
	...
	public V putIfAbsent(final K key, final Function<? super K, ? extends V> putIfNotPresent) {
		return table.putIfAbsent(hash(key), key, keyEq, new HashNodeFactory<K, N>() {
	...
	public ScalarMap<K, V> copy() {
		return new ScalarHashMap<K, V, N>(keyHasher, rehasher, keyEq, entryEq, nodeFactory, table.copy()) ;
	}
