...
public abstract class AbstractHashMap<K, V, N extends Map.Entry<K, V>> implements ArbitraryMap<K, V> {
	...
	public int remove(K key, V val) {
		return table.remove(hash(key), entry(key, val), entryEq) ;
	}
	@Override
	public V removeAndReturnFirst(K key) {
		return table.removeAndReturnFirst(hash(key), key, keyEq, valProj()) ;
	}
	@Override
	public Iterable<Entry<K, V>> removeAndReturn(K key, V val) {
		return table.removeAndReturn(hash(key), entry(key, val), entryEq, entryProj()) ;
	}
	@Override
	public Iterable<Entry<K, V>> removeAndReturn(K key) {
		return table.removeAndReturn(hash(key), key, keyEq, entryProj()) ;
	}
	@Override
	public int remove(K key) {
		return table.remove(hash(key), key, keyEq) ;
	}
	@Override
	public boolean contains(K key, V val) {
		return table.contains(hash(key), entry(key, val), entryEq) ;
	}
	@Override
	public boolean contains(K key) {
		return table.contains(hash(key), key, keyEq) ;
	}
	@Override
	public int count(K key, V val) {
		return table.count(hash(key), entry(key, val), entryEq) ;
	}
	@Override
	public int count(K key) {
		return table.count(hash(key), key, keyEq) ;
	}
	@Override
	public Iterable<Entry<K, V>> entries(final K key) {
		...
		return new Iterable<Entry<K, V>>() {
			...
			public Iterator<Entry<K, V>> iterator() {
				return table.find(hash, key, keyEq, keyProj(), entryProj()) ;
			}
		} ;
	}
	@Override
	...
	public V first(K key) {
		return table.first(hash(key), key, keyEq, valProj()) ;
	}
	@Override
	...
	public Iterable<V> values() {
		return new AbstractIterable<V>() {
			...
			public Iterator<V> iterator() {
				return table.all(keyProj(), keyEq, valProj()) ;
			}
		} ;
	}
	@Override
	public Iterable<V> values(final K key) {
		...
		return new Iterable<V>() {
			@Override
			public Iterator<V> iterator() {
				return table.find(hash, key, keyEq, keyProj(), valProj()) ;
			}
		} ;
	}
	...
	public Iterator<Entry<K, V>> clearAndReturn() {
		return table.clearAndReturn(entryProj()) ;
	}
	...
	protected abstract class AbstractKeySet extends AbstractIterable<K> implements ArbitrarySet<K> {
		...
		public Iterator<K> clearAndReturn() {
			return table.clearAndReturn(Functions.<K, Entry<K, V>>getMapEntryKeyProjection()) ;			
		}
		...
		public Iterable<K> all() {
			return new AbstractIterable<K>() {
				...
				public Iterator<K> iterator() {
					return table.all(keyProj(), keyEq, keyProj()) ;
				}
			} ;
		}
		...
		public Iterable<K> all(final K key) {
			...
			return new AbstractIterable<K>() {
				@Override
				public Iterator<K> iterator() {
					return table.find(hash, key, keyEq, keyProj(), keyProj()) ;
				}
			} ;
		}
		...
		public K first(final K key) {
			return table.first(hash(key), key, keyEq, keyProj()) ;
		}
		...
		public Iterable<K> removeAndReturn(K key) {
			return table.removeAndReturn(hash(key), key, keyEq, keyProj());
		}
		
		@Override
		public K removeAndReturnFirst(K key) {
			return table.removeAndReturnFirst(hash(key), key, keyEq, keyProj()) ;
		}
	...
	protected abstract class AbstractEntrySet extends AbstractIterable<Entry<K, V>> implements ArbitrarySet<Entry<K, V>> {
		...
		public Iterable<Entry<K, V>> all() {
			return new AbstractIterable<Entry<K,V>>() {
				@Override
				public Iterator<Entry<K, V>> iterator() {
					return table.all(keyProj(), keyEq, entryProj()) ;
				}
			} ;
		}
		...
		public Iterable<Entry<K, V>> all(final Entry<K, V> entry) {
			...
			return new AbstractIterable<Entry<K,V>>() {
				@Override
				public Iterator<Entry<K, V>> iterator() {
					return table.find(hash, entry, entryEq, nodeProj(), entryProj()) ;
				}
			} ;
		}
		...
		public Entry<K, V> first(Entry<K, V> entry) {
			return table.first(hash(entry.getKey()), entry, entryEq, nodeProj()) ;			
		}
