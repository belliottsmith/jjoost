...
public final class CharScheme extends NodeScheme<CharScheme.Char> {
	...
	@Override
	public NodeMap<Char> merge(@SuppressWarnings("unchecked") NodeMap<Char>... merge) {
		if (merge.length <= 1) {
			if (merge.length == 0) {
				return EMPTY;
			} else {
	...
	static final class CharMapBuilder implements NodeMapBuilder<Char> {
		...
		@Override
		public NodeMapBuilder<Char> bind(Group<Char> s, Node<Char> n, IdCapture capture) {
			if (s instanceof CharGroup) {
				...
				shift(i += 1);
				c++;
				ms[i] = new Maplet(n, capture, g.first, g.last);
				return this;
			} else {
				throw new IllegalStateException();
			}
	...
	static final class CharMap implements NodeMap<Char>{
			...
			if (that1 instanceof CharMap) {
				...
				while (lr != null & rr != null) {
					if (lr.last < rr.first) {
						...
						lrout[lrc++] = new NodeIntersect<Char>(intersect, lr.next, lr.capture, rr.next, rr.capture.shift(offset));
						if (lr.first < rr.first) {
							lout[lc++] = new Maplet(lr.next, lr.capture, lr.first, (char)(intersect.first - 1));							
						}
						if (rr.first < lr.first) {
							rout[rc++] = new Maplet(rr.next, rr.capture.shift(offset), rr.first, (char)(intersect.first - 1));
						}
						if (lr.last > rr.last) {
							lr = new Maplet(lr.next, lr.capture, (char)(intersect.last + 1), lr.last);
							rr = j < rin.length ? rin[j++] : null;
						} else if (lr.last < rr.last) {
							rr = new Maplet(rr.next, rr.capture.shift(offset), (char)(intersect.last + 1), rr.last);
							lr = i < lin.length ? lin[i++] : null;
						} else {
							lr = i < lin.length ? lin[i++] : null;
							rr = j < rin.length ? rin[j++] : null;
		...
		public NodeMap<Char> replace(Map<Node<Char>, Node<Char>> replace, int offset) {
			...
			for (int i = 0 ; i != ms.length ; i++) {
				...
				if (repl != null) {
					ms[i] = new Maplet(repl, m.capture.shift(offset), m.first, m.last);
				}
			}
		...
		public NodeMap<Char> addCapture(IdCapture capture) {
			...
			for (int i = 0 ; i != ms.length ; i++) {
				...
				ms[i] = new Maplet(m.next, m.capture.append(capture, false, 0), m.first, m.last);
			}
			return new CharMap(ms);
		}
	...
	static final class Maplet extends CharGroup implements NodeMapEntry<Char> {
		...
		private IdCapture capture;
		public Maplet(Node<Char> next, IdCapture capture, char start, char end) {
			super(start, end);
			this.next = next;
