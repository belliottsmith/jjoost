...
public class InlineListHashMap<K, V, N extends Entry<K, V>> extends AbstractHashMap<K, V, N> implements ListMap<K, V> {
	...
	protected InlineListHashMap(
			...
			HashMapNodeFactory<K, V, N> nodeFactory, HashTable<N> table) {
	...
	public V put(K key, V val) {
		...
		return table.put(n, n, entryEq, valProj()) ;
	}

	@Override
	public V putIfAbsent(K key, V val) {
		final N n = nodeFactory.node(hash(key), key, val) ;
		return table.putIfAbsent(n, n, entryEq, valProj()) ;
	}
	...
	public ListMap<K, V> copy() {
		return new InlineListHashMap<K, V, N>(keyHasher, rehasher, keyEq, entryEq, nodeFactory, table.copy()) ;
	}
		...
		@Override public Iterable<K> unique() { 
			return new Iterable<K>() {
				...
				public Iterator<K> iterator() {
					return table.unique(keyProj(), keyEq, keyEq.getKeyEquality(), keyProj()) ;
				}
			} ;
		}
		@Override
	...
	final class EntrySet extends AbstractEntrySet implements ListSet<Entry<K, V>> {
		...
		@Override public Iterable<Entry<K, V>> unique() {
			return new Iterable<Entry<K, V>>() {
				...
				public Iterator<Entry<K, V>> iterator() {
					return table.unique(nodeProj(), entryEq, entryEq, entryProj()) ;
				}
			};
		}
		@Override
		public Entry<K, V> put(Entry<K, V> entry) {
			...
			final N n = nodeFactory.node(hash(key), key, val) ;
			return table.put(n, n, entryEq, entryProj()) ;
		}
		@Override
		public Entry<K, V> putIfAbsent(Entry<K, V> entry) {
			...
			final V val = entry.getValue() ;
			final N n = nodeFactory.node(hash(key), key, val) ;
			return table.putIfAbsent(n, n, entryEq, entryProj()) ;
		}
		@Override
		public boolean permitsDuplicates() {
			return true ;
